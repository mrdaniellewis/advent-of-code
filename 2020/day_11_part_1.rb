  ...
  current.each_with_index do |row, row_index|
    row.each_with_index do |seat, seat_index|
      ...
      for i in [row_index - 1, row_index, row_index + 1]
        ...
        for j in [seat_index - 1, seat_index, seat_index + 1]
          next if j < 0
          next if i == row_index && j == seat_index

          adjacent += 1 if current[i]&.[](j)
          # puts "#{row_index} #{seat_index} #{adjacent} #{i} #{j} #{current[i]&.[](j)} #{current[i]}"

          # Optimization
          break_loop = true if !seat && adjacent == 1
          break_loop = true if seat && adjacent == 4
          break if break_loop
        end
        break if break_loop
      end

      # puts "#{row_index} #{seat_index} #{adjacent}" if rounds == 2

      round[row_index][seat_index] = false if seat && adjacent >= 4
