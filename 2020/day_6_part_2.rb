...
DATA.each_line do |line|
  questions << '' if line.strip == ''

  questions[questions.length - 1] += line
end

puts(questions.map do |text|
  text.strip.split(/\s+/).reduce(nil) do |memo, line|
    next line.chars.uniq unless memo

    memo.intersection(line.chars.uniq)
  end.length
end.reduce(0, :+))
