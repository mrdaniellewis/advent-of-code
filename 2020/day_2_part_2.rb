...
DATA.each_line do |line|
  parts = /^(?<min>\d+)-(?<max>\d+) (?<letter>\w): (?<password>\w+)$/.match(line)
  items << parts.named_captures
end

puts(items.reduce(0) do |sum, item|
  one = item["password"].chars[item["min"].to_i - 1] == item["letter"]
  two = item["password"].chars[item["max"].to_i - 1] == item["letter"]
  valid = (one || two) && one != two
  valid ? sum + 1 : sum
end)
