...
DATA.each_line do |line|
  line = line.strip
  next passports << {} if line == ''

  current = passports.last
...
def validate_byr?(value)
  value && /^\d{4}$/.match?(value) && (1920..2002).include?(value.to_i)
end

def validate_iyr?(value)
  value && /^\d{4}$/.match?(value) && (2010..2020).include?(value.to_i)
end

def validate_eyr?(value)
  value && /^\d{4}$/.match?(value) && (2020..2030).include?(value.to_i)
end

def validate_hgt?(value)
  return false unless value

  parts = /^(?<number>\d+)(?<unit>cm|in)$/.match(value)
  return false unless parts
  return true if parts.named_captures['unit'] == 'cm' && (150..193).include?(parts.named_captures['number'].to_i)
  return true if parts.named_captures['unit'] == 'in' && (59..76).include?(parts.named_captures['number'].to_i)

  false
end

def validate_hcl?(value)
  value && /^#[a-f0-9]{6}/i.match?(value)
end

def validate_ecl?(value)
  value && %w[amb blu brn gry grn hzl oth].include?(value)
end

def validate_pid?(value)
  value && /^[0-9]{9}$/.match?(value)
end

puts(passports.select do |passport|
  fields.all? { |name| send("validate_#{name}?", passport[name]) }
end.length)
