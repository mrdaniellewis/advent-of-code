...
rules.each do |rule|
  contents = rule.scan(/(?:(\d+) )?(\w+ \w+) bag/)
  bags[contents[0][1]] = Hash[contents.slice(1..).reject { |i| i[0].nil? }.map(&:reverse)].transform_values(&:to_i)
end

def gold?(bags, name)
  return true if bags[name].keys.include?('shiny gold')

  bags[name].keys.any? { |n| gold?(bags, n) }
end

puts(bags.keys.reduce(0) do |count, bag|
  count + (gold?(bags, bag) ? 1 : 0)
end)
