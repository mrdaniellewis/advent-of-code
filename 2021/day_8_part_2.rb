...
DATA.each_line do |line|
  test, actual = line.split("|")
  samples << [
    test.split(" ").map(&:strip),
    actual.split(" ").map(&:strip)
  ]
end

# Valid segments to produce each number
SEGMENTS = [
  Set[0, 1, 2, 4, 5, 6],
  Set[2, 5],
  Set[0, 2, 3, 4, 6],
...
puts(samples.map do |(letters, actual)|
  tests = letters.map { |word| word.split("") }
  # brute it: try every permutation of letters and see if all
  # the test entries match an allowed set of segments
  combination = %w[a b c d e f g].permutation(7).find do |position|
    tests.all? do |test|
      SEGMENTS.include?(Set.new(test.map { |l| position.index(l) }))
    end
  end

  actual.map do |word|
    SEGMENTS.index(Set.new(word.split("").map { |l| combination.index(l) }))
  end.join("").to_i
end.sum)
