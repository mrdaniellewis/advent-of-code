...
def visit(pos, grid, start_pos, end_pos)
...
def moves(grid, start, target)
  ...
  min = 0
    ...
    loop do
      queue = queues[min]
      queue.each do |pos|
        new_positions = visit(pos, positions[min], start, target)

        queues[min + 1] ||= Set.new
        new_positions.each do |p|
          throw :done if p == target
...
goal, goal_grid = moves(grid, START_POS, END_POS)
start, start_grid = moves(goal_grid, END_POS, START_POS)
final, = moves(start_grid, START_POS, END_POS)
