...
yparts = 4.times.map { _1 * xmax / 3 }
xparts = 5.times.map { _1 * ymax / 4 }
    ...
    loop do
      ...
      if pos[Y] > ymax
        ...
        when yparts[0]..yparts[1]
          pos = Vector[xmax - (yparts[1] - pos[X]), 0]
        when yparts[1] + 1..yparts[2]
          dir = RIGHT * dir
          pos = Vector[yparts[1], xparts[3] + 1 + (pos[X] - (yparts[1] + 1))]
        when yparts[2] + 1..yparts[3]
          dir = RIGHT * dir
          pos = Vector[yparts[2], xparts[1] + 1 + (pos[X] - (yparts[2] + 1))]
        else
          raise "fail"
        end
      elsif pos[Y] < ymin
        ...
        when yparts[0]..yparts[1]
          dir = RIGHT * dir
          pos = Vector[0, xparts[2] - (xparts[1] - pos[X])]
        when yparts[1] + 1..yparts[2]
          dir = RIGHT * dir
          pos = Vector[0, xparts[3] + 1 + (pos[X] - (yparts[1] + 1))]
        when yparts[2] + 1..yparts[3]
          pos = Vector[pos[X] - (yparts[2] + 1), ymax]
        else
          raise "fail"
        end
      elsif pos[X] > xmax
        ...
        when xparts[0]..xparts[1]
          dir = FLIP * dir
          pos = Vector[yparts[2], xparts[3] - pos[Y]]
        when xparts[1] + 1..xparts[2]
          dir = LEFT * dir
          pos = Vector[yparts[2] + 1 + pos[Y] - (xparts[1] + 1), ymax]
        when xparts[2] + 1..xparts[3]
          dir = FLIP * dir
          pos = Vector[xmax, xparts[1] - (pos[Y] - (xparts[2] + 1))]
        when xparts[3] + 1..xparts[4]
          dir = LEFT * dir
          pos = Vector[yparts[1] + 1 + (pos[Y] - (xparts[3] + 1)), xparts[3]]
        else
          raise "fail"
        end
      elsif pos[X] < xmin
        ...
        when xparts[0]..xparts[1]
          dir = FLIP * dir
          pos = Vector[0, xparts[3] - pos[Y]]
        when xparts[1] + 1..xparts[2]
          dir = LEFT * dir
          pos = Vector[pos[Y] - (xparts[1] + 1), xparts[2] + 1]
        when xparts[2] + 1..xparts[3]
          dir = FLIP * dir
          pos = Vector[yparts[1] + 1, xparts[3] - pos[Y]]
        when xparts[3] + 1..xparts[4]
          dir = LEFT * dir
          pos = Vector[yparts[1] + 1 + pos[Y] - (xparts[3] + 1), 0]
        else
          raise "fail"
        end
      end
